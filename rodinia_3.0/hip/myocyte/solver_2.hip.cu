...
__global__ void solver_2(hipLaunchParm lp, 	int workload,
											int xmax,
	...
	// CUDA indexes
	bx = hipBlockIdx_x;																// get current horizontal block index (0-n)
	tx = hipThreadIdx_x;																// get current horizontal thread index (0-n)
	...
	if(tid<workload){
		...
		for(k=1; k<(xmax+1); k++) {											// start after initial value
			...
			for (j = 0; j < ATTEMPTS; j++) {
				...
				scale_fina = min1(max1(scale_min,MIN_SCALE_FACTOR), MAX_SCALE_FACTOR);
