		...
		char *argv [])
{
		...
		if(strcmp(argv[dim_cpu.cur_arg], "-boxes1d")==0){
			...
			if(argc>dim_cpu.cur_arg+1){
				...
				if(isInteger(argv[dim_cpu.cur_arg+1])==1){
					...
					if(rnd_seed<0){
						printf("ERROR: Wrong value to -rnd_seed parameter, cannot be <=0\n");
						return 0;
					}
					dim_cpu.cur_arg = dim_cpu.cur_arg+1;
				}
				// value is not a number
					...
					printf("ERROR: Value to -rnd_seed parameter in not a number\n");
					return 0;
				}
			}
			// value not provided
			else{
				printf("ERROR: Missing value to -rnd_seed parameter\n");
				return 0;
			}
		}
		// unknown
	...
	//srand(time(NULL));
	srand(rnd_seed);//initialize with a seed so that results between two consecutive runs can be compared
/*
	FILE *fp_rv, *fp_qv;
	fp_rv=fopen("rv_cpu.bin", "wb");
	fp_qv=fopen("qv_cpu.bin", "wb");
*/	
	// input (distances)
...
/*
	fwrite(&(rv_cpu[i].v), sizeof(double), 1, fp_rv);
	fwrite(&(rv_cpu[i].x), sizeof(double), 1, fp_rv);
	fwrite(&(rv_cpu[i].y), sizeof(double), 1, fp_rv);
	fwrite(&(rv_cpu[i].z), sizeof(double), 1, fp_rv);
*/
	}
//	fclose(fp_rv);
	// input (charge)
...
//	fwrite(&(qv_cpu[i]), sizeof(double), 1, fp_qv);
	}
//	fclose(fp_qv);
	// output (forces)
