...
) 
{
  ...
  else if ( by == hipGridDim_y - 1 ){
  south[ty][tx] = J_cuda[cols * BLOCK_SIZE * (hipGridDim_y - 1) + BLOCK_SIZE * bx + cols * ( BLOCK_SIZE - 1 ) + tx];
  }
   __syncthreads();
  ...
  else if ( bx == hipGridDim_x - 1 ){
  east[ty][tx] = J_cuda[cols * BLOCK_SIZE * by + BLOCK_SIZE * ( hipGridDim_x - 1) + cols * ty + BLOCK_SIZE-1];
  }
...
) 
{
	...
	south_c[ty][tx] = C_cuda[cols * BLOCK_SIZE * (hipGridDim_y - 1) + BLOCK_SIZE * bx + cols * ( BLOCK_SIZE - 1 ) + tx];
	}
	__syncthreads();
	...
	if ( bx == hipGridDim_x - 1 ){
	east_c[ty][tx] = C_cuda[cols * BLOCK_SIZE * by + BLOCK_SIZE * ( hipGridDim_x - 1) + cols * ty + BLOCK_SIZE-1];
	}
