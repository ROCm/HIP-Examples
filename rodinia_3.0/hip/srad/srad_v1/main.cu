...
int main(int argc, char *argv []){
	...
	hipLaunchKernel(HIP_KERNEL_NAME(extract), dim3(blocks), dim3(threads), 0, 0, 	Ne,
									d_I);
	...
	// fflush(NULL);
		...
		hipLaunchKernel(HIP_KERNEL_NAME(prepare), dim3(blocks), dim3(threads), 0, 0, 	Ne,
										d_I,
										d_sums,
										d_sums2);
		...
		while(blocks2.x != 0){
			...
			// run kernel
			hipLaunchKernel(HIP_KERNEL_NAME(reduce), dim3(blocks2), dim3(threads), 0, 0, 	Ne,
											no,
											mul,
											d_sums, 
											d_sums2);
		...
		// execute srad kernel
		hipLaunchKernel(HIP_KERNEL_NAME(srad), dim3(blocks), dim3(threads), 0, 0, 	lambda,									// SRAD coefficient 
		...
		hipLaunchKernel(HIP_KERNEL_NAME(srad2), dim3(blocks), dim3(threads), 0, 0, 	lambda,									// SRAD coefficient 
									Nr,										// # of rows in input image
									Nc,										// # of columns in input image
									Ne,										// # of elements in input image
									d_iN,									// indices of North surrounding pixels
	...
	hipLaunchKernel(HIP_KERNEL_NAME(compress), dim3(blocks), dim3(threads), 0, 0, 	Ne,
									d_I);
