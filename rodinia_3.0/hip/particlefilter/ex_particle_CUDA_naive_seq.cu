...
__global__ void kernel(hipLaunchParm lp,  double * arrayX, double * arrayY, double * CDF, double * u, double * xj, double * yj, int Nparticles){
	...
	int block_id = hipBlockIdx_x;// + hipGridDim_x * hipBlockIdx_y;
...
void particleFilter(int * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles){
	...
	//fprintf(stderr, "TIME TO GET NEIGHBORS TOOK: %f\n", elapsed_time(start, get_neighbors));
	...
	//fprintf(stderr, "TIME TO GET WEIGHTSTOOK: %f\n", elapsed_time(get_neighbors, get_weights));
	...
	for(k = 1; k < Nfr; k++){
		...
		//fprintf(stderr, "TIME TO SET ARRAYS TOOK: %f\n", elapsed_time(get_weights, set_arrays));
		//apply motion model
		...
		//fprintf(stderr,"TIME TO SET ERROR TOOK: %f\n", elapsed_time(set_arrays, error));
		for(x = 0; x < Nparticles; x++){
			...
			for(y = 0; y < countOnes; y++){
				...
				ind[y] = (int)fabs(  (float) (indX*IszY*Nfr + indY*Nfr + k));
				if(ind[y] >= max_size)
					ind[y] = 0;
			}
		...
		//fprintf(stderr, "TIME TO GET LIKELIHOODS TOOK: %f\n", elapsed_time(error, likelihood_time));
		...
		//fprintf(stderr, "TIME TO GET EXP TOOK: %f\n", elapsed_time(likelihood_time, exponential));
		double sumWeights = 0;	
		for(x = 0; x < Nparticles; x++){
			sumWeights += weights[x];
		}
		long long sum_time = get_time();
		//fprintf(stderr, "TIME TO SUM WEIGHTS TOOK: %f\n", elapsed_time(exponential, sum_time));
		for(x = 0; x < Nparticles; x++){
				weights[x] = weights[x]/sumWeights;
		}
		long long normalize = get_time();
		//fprintf(stderr, "TIME TO NORMALIZE WEIGHTS TOOK: %f\n", elapsed_time(sum_time, normalize));
		xe = 0;
		ye = 0;
		...
		//fprintf(stderr, "TIME TO MOVE OBJECT TOOK: %f\n", elapsed_time(normalize, move_time));
		printf("XE: %lf\n", xe);
		printf("YE: %lf\n", ye);
		...
		//fprintf(stderr, "TIME TO CALC CUM SUM TOOK: %f\n", elapsed_time(move_time, cum_sum));
		double u1 = (1/((double)(Nparticles)))*randu(seed, 0);
		for(x = 0; x < Nparticles; x++){
			u[x] = u1 + x/((double)(Nparticles));
		}
		long long u_time = get_time();
		//fprintf(stderr, "TIME TO CALC U TOOK: %f\n", elapsed_time(cum_sum, u_time));
		long long start_copy = get_time();
#ifdef PROFILING
		mytimer->Reset("CPU TO GPU DATA TRANSFER");
		mytimer->Start();
#endif
		...
		//fprintf(stderr, "SENDING TO GPU TOOK: %lf\n", elapsed_time(start_copy, end_copy));
		//fprintf(stderr,"CUDA EXEC TOOK: %lf\n", elapsed_time(end_copy, start_copy_back));
		//fprintf(stderr,"SENDING BACK FROM GPU TOOK: %lf\n", elapsed_time(start_copy_back, end_copy_back));
		long long xyj_time = get_time();
		//fprintf(stderr,"TIME TO CALC NEW ARRAY X AND Y TOOK: %f\n", elapsed_time(u_time, xyj_time));
		...
		//fprintf(stderr,"TIME TO RESET WEIGHTS TOOK: %f\n", elapsed_time(xyj_time, reset));
	}
