...
#ifdef __KALMAR_CC__
//#define IMUL(a, b) nvvm_mul24_i(a, b)
#define IMUL(a, b) a*b
//#define IMUL(a, b)
#else
...
__global__ void histogram1024Kernel(hipLaunchParm lp,  unsigned int *d_Result, float *d_Data, float minimum, float maximum, int dataN){
    ...
    const int   numThreads = 0;// = IMUL(hipBlockDim_x, hipGridDim_x);
    ...
    const unsigned int threadTag = hipThreadIdx_x << (32 - WARP_LOG_SIZE);
    ...
    const int warpBase = 0;// = IMUL(hipThreadIdx_x >> WARP_LOG_SIZE, BIN_COUNT);
    
    //Clear shared memory buffer for current thread block before processing
    for(int pos = hipThreadIdx_x; pos < BLOCK_MEMORY; pos += hipBlockDim_x)
       s_Hist[pos] = 0;
    ...
    for(int pos = hipThreadIdx_x; pos < BIN_COUNT; pos += hipBlockDim_x){
        unsigned int sum = 0;

        for(int base = 0; base < BLOCK_MEMORY; base += BIN_COUNT)
            sum += s_Hist[base + pos] & 0x07FFFFFFU;

//	    d_Result = d_Result + pos ;
    ...
    int dataN)
{
	...
	//hipLaunchKernel(histogram1024Kernel, dim3(BLOCK_N), dim3(THREAD_N), 0, 0, d_Result1024,d_Data,minimum,maximum,dataN );
