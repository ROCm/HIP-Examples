...
#endif
{
    ...
    const unsigned int threadTag = hipThreadIdx_x << (32 - BUCKET_WARP_LOG_SIZE);
    const int warpBase = (hipThreadIdx_x >> BUCKET_WARP_LOG_SIZE) * DIVISIONS; 
    const int numThreads = hipBlockDim_x * hipGridDim_x;
	for (int i = hipThreadIdx_x; i < BUCKET_BLOCK_MEMORY; i += hipBlockDim_x)
		s_offset[i] = 0; 

	__syncthreads(); 

	for (int tid = hipBlockIdx_x * hipBlockDim_x + hipThreadIdx_x; tid < size; tid += numThreads) {
		float elem = input[tid]; 

		int idx1  = DIVISIONS/2 - 1; 
		int jump = DIVISIONS/4;
#ifndef USE_TEXTURES
		float piv = pivotpoints[idx1] ;
#else 
		float piv = tex1Dfetch(texPivot, idx1); //s_pivotpoints[idx]; 
#endif
		while(jump >= 1){
			idx1 = (elem < piv) ? (idx1 - jump) : (idx1 + jump);
		#ifndef USE_TEXTURES
			piv = pivotpoints[idx1];
		#else	
			piv = tex1Dfetch(texPivot, idx1); //s_pivotpoints[idx]; 
		#endif
			jump /= 2; 
		}
		idx1 = (elem < piv) ? idx1 : (idx1 + 1); 

		indice[tid] = (addOffset(s_offset + warpBase, idx1, threadTag) << LOG_DIVISIONS) + idx1;  //atomicInc(&offsets[idx], size + 1);
	}

	__syncthreads(); 

	int prefixBase = hipBlockIdx_x * BUCKET_BLOCK_MEMORY; 
