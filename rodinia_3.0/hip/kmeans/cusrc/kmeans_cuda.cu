		   ...
		   )
{
	...
	cudChannelFormatDesc chDesc1 = cudaCreateChannelDesc<float>();
    ...
    // THis appears to have a bug - using constant memory does not work, only
	cudaError_t err = cudaMemcpyToSymbol(c_clusters,clusters[0],nclusters*nfeatures*sizeof(float),0,cudaMemcpyHostToDevice);
